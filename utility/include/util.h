...
namespace util {
    ...
    /**
     * Generates a 64 bit {replication+consecutive} block id.
     * This method only uses the first 8 bytes of the UUID generated by boost.
     * Note that the highest bit is set to 0 which indicates the block is not EC-based.
     * @param uuid where to put the uuid.
     */
    inline void generate_block_id(std::uint64_t &uuid) {
        auto bytes = boost::uuids::random_generator()();
        memcpy((&uuid), &bytes, sizeof(std::uint64_t));
        // mask with all 1s (63 1s). This effectively sets the 64th bit to 0.
        uuid = uuid & ((1ull << 63) - 1);
    }
