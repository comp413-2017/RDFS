    ...
    while ((c = getopt(argc, argv, "v:p:x:b:")) != -1) {
        switch (c) {
            ...
            case '?':
                switch (optopt) {
                    case 'v':
                        LOG(ERROR) << "Option -v requires an argument specifying a verbosity level.";
                        return -1;
                    case 'p':
                        LOG(ERROR) << "Option -p requires an argument specifying an ipc port.";
                        return -1;
                    case 'x':
                        LOG(ERROR) << "Option -x requires an argument specifying a transfer server port.";
                        return -1;
                    case 'b':
                        LOG(ERROR) << "Option -b requires an argument specifying the backing storage device.";
                        return -1;
                    default:
                        if (isprint(optopt)) {
