 ...
 */
static inline int parse_cmdline_options(int argc, char *argv[], int *xferPort, int *ipcPort,
                                        int *verbosity, std::string &backingStore) {
  int c;
  char buf[64];
      ...
      case '?':
        switch (optopt) {
          ...
          case 'p':LOG(ERROR) << "Option -p requires an argument specifying an ipc port.";
            return -1;
          case 'x':LOG(ERROR) << "Option -x requires an argument specifying a transfer server port.";
            return -1;
          case 'b':LOG(ERROR) << "Option -b requires an argument specifying the backing storage device.";
            return -1;
          default:
            if (isprint(optopt)) {
...
int main(int argc, char *argv[]) {
  ...
  if (parse_cmdline_options(argc, argv, &xferPort, &ipcPort, &verbosity, backingStore) != 0) {
  ...
  auto dncli = std::make_shared<zkclient::ZkClientDn>("127.0.0.1",
                                                      ...
                                                      total_disk_space,
                                                      ipcPort,
                                                      xferPort); // TODO: Change the datanode id
