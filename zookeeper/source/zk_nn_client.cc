...
namespace zkclient{
    ...
    bool ZkNnClient::previousBlockComplete(uint64_t prev_id) {
		...
		LOG(INFO) << "JOSEPH: Checking if block " << prev_id << " has been complete";
		if (zk->get_children(BLOCK_LOCATIONS + block_id_str, children, error_code)) {
			if (children.size() >= MIN_REPLICATION){
				LOG(INFO) << "JOSEPH: Block " << prev_id << " has been complete with " << MIN_REPLICATION << " copies";
				return true;
			}
			LOG(ERROR) << "JOSEPH: Block " << prev_id << " has failed with " << MIN_REPLICATION << " copies";
			return false;
		}
		LOG(ERROR) << "JOSEPH: Block " << prev_id << " get children failed " << BLOCK_LOCATIONS + block_id_str;
		return true;
    }
